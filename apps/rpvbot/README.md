...
- **`/soul <@username>`.** Anyone can render a member's soul as a Telegram-styled character card: the class title, the six stats as 10-cell bars, essence, traits, quirks, skills, and catchphrase. The free-text `notes` field is **not** shown — it's an internal running memory for the souls cron and `/rpv` context, not for public display. The body is laid out with Telegram's legacy `Markdown` parse mode; member-written card text is escaped before interpolation so an evolving soul can't forge styling. The card text is Spanish on Mon–Thu / Sat–Sun and English on Fridays, like the resume. It's a read-only sheet lookup — no Gemini call, no rate limit. If the `@username` is unknown, or the member only has a legacy free-text soul (not yet regenerated as a card), Capitán RPV replies that he hasn't chronicled them yet.
