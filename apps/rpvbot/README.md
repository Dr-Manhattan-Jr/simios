...
Telegram bot — **Capitán RPV**, the chronicler of the group. Five features:
...
- **Daily souls cron.** Every day at 02:00 Europe/Madrid, the bot reads yesterday's messages, groups them by member, and incrementally updates a per-member "soul" — a **dark-fantasy RPG character card**. The card has six fixed numeric stat axes (verbosity, humor, chaos, wisdom, horniness, menace, each 1–10, scored relative to a normal group member) plus free-form imaginative fields: a fantasy class `title`, an `essence`, `traits`, `quirks`, funny RPG-style `skills`, an optional `catchphrase`, and a free-text `notes` field — a looser, capped running memory (in-jokes, evolving context, recurring dynamics) that the rigid card slots can't hold. Update is `previous_card + new_messages → new_card` via Gemini structured-JSON synthesis, stored as JSON in the `rpv_souls` tab; every field, including `notes`, is re-synthesised (evolved, not reset) each run. The whole card — stats, fields, and `notes` — is injected as background context into `/rpv` question answers (see above), and is viewable on demand via `/soul` (see below).
- **`/soul <@username>`.** Anyone can render a member's soul as a Telegram-styled character card: the class title, the six stats as 10-cell bars, essence, traits, quirks, skills, and catchphrase. The free-text `notes` field is **not** shown — it's an internal running memory for the souls cron and `/rpv` context, not for public display. The card header (title, member, catchphrase) is always visible; the rest of the card sits inside a **MarkdownV2 expandable blockquote**, so the message arrives collapsed and each reader taps "show more" to see the full soul. Member-written card text is escaped before interpolation so an evolving soul can't forge styling or break parsing. The card text is Spanish on Mon–Thu / Sat–Sun and English on Fridays, like the resume. It's a read-only sheet lookup — no Gemini call, no rate limit. If the `@username` is unknown, or the member only has a legacy free-text soul (not yet regenerated as a card), Capitán RPV replies that he hasn't chronicled them yet.
...
9. `/soul <@username>` command: look up the member's row in `rpv_souls` by username (case-insensitive), render the stored card as a Telegram-styled message. No Gemini call.
...
Covers `/rpv` argument parsing (count vs question), `/soul` argument parsing (@username with/without `@`), the `/soul` card renderer (stat bars, ES/EN labels, MarkdownV2 escaping of member-written text, the expandable-blockquote wrapping, legacy-soul fallback), question sanitisation (injection-shaped payloads), day-of-week language switching, previous-calendar-day window calculation (with DST edge cases), transcript rendering, sticker text formatting, souls per-user grouping, the hostile-input defence in the question system prompt, the image record schema, the `[photo: …]` / `[sticker: …]` token builder, and the OCR cron's pending-image selection.
