...
- **[luditabot](apps/luditabot)** — listens for `ludita`, `luditas`, `luddite`, or `luddites` and replies with a Gemini-generated image of a monkey opposing technology or modern advances. Trailing text becomes visual inspiration for the prompt. See [apps/luditabot/README.md](apps/luditabot/README.md) for full setup.
...
- run a Telegram bot that turns trigger words into AI-generated images → [apps/tractorbot/README.md](apps/tractorbot/README.md) or [apps/luditabot/README.md](apps/luditabot/README.md).
...
# also: pnpm -F tractorbot dev, pnpm -F luditabot dev, pnpm -F los_piratas_bot dev, pnpm -F rpvbot dev
```
...
Push to `main`. Railway watches the GitHub repo and rebuilds the Dockerfile of every affected app. No CLI deploy step. After a push, `railway logs --service <name>` (e.g. `ciclobot`, `tractorbot`, `luditabot`, `rpvbot`, or the los_piratas_bot service ID — see its README) confirms the rollout.
